...
* **Groq SDK**: Connects securely to the **Llama-3.3-70b-versatile** model via Groq's high-speed inference engine to perform advanced semantic fact-checking.
...
3. **AI Evaluation (Groq)**: The backend cleans the input text and sends a structured prompt to the Llama-3.3 model on Groq. The prompt requests a strict JSON output matching this schema:
   ```json
   {
